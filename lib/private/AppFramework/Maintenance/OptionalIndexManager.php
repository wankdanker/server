...
class OptionalIndexManager {
	...
	public function getPending(): array {
		...
		foreach ($context->getOptionalIndexes() as $optionalIndex) {
			try {
				$class = $this->serverContainer->query($optionalIndex['class']);
			} catch (QueryException $e) {
				$this->logger->info('Could not initialize ' . $optionalIndex['class'] . ' for ' . $optionalIndex['appId']);
				continue;
			}

			if (!($class instanceof IOptionalIndex)) {
				$this->logger->info($optionalIndex['class'] . ' is not an instance of ' . IOptionalIndex::class);
				continue;
			}

			if ($class->exists()) {
				$this->logger->debug($optionalIndex['class'] . ' is already added');
				continue;
			}
